...
enum APIURL {
    case forecast(latitude: Int, longitude: Int, key: String)
    case current(latitude: Int, longitude: Int, key: String)
    case icon(icon: String)
    
    var urlString: String {
        switch self {
        case .forecast(let latitude, let longitude, let key):
            return "api.openweathermap.org/data/2.5/forecast?lat=\(latitude)&lon=\(longitude)&appid=\(key)"
